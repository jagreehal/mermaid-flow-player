...
Extend Mermaid Flow Player with plugins that hook into the animation lifecycle. To react to steps without writing a plugin (e.g. play audio or speak with the Web Speech API), use [config-level hooks](../event-hooks/) instead.
