...
- [Plugin System](../plugins/) — `api.beforeStep(step)` and `api.afterStep(step)` run before and after each step.

Plugins are useful when you want to package your behavior (e.g. analytics or sound) and reuse it across multiple players.

## Next steps

- [Narration](../narration/) — Use `narrationTarget` and step `note` for on-screen text.
- [Plugin System](../plugins/) — Lifecycle hooks and extensions.
